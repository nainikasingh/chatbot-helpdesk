...
def fuzzy_extract_sheet_names(query: str, available_sheets: list, threshold: int = 60):
    ...
    # Debugging: Print cleaned versions
    logging.basicConfig(level=logging.INFO)
    logging.info(f"🟢 Raw Query: {query}")
    logging.info(f"🟢 Cleaned Query: {query_clean}")
    logging.info(f"🟢 Available Sheets (Cleaned): {sheets_clean}")
    ...
    detected_sheets = [original for original, cleaned in zip(available_sheets, sheets_clean) 
                   if any(fuzz.partial_ratio(cleaned, query_clean) >= threshold for _, score in matches)]
    
    # Debugging: Print detected sheets
    logging.info(f"🟢 Detected Sheets (Threshold {threshold}+): {detected_sheets}\n")

    return detected_sheets

detected_sheets = fuzzy_extract_sheet_names(query, available_sheets)
if not detected_sheets:
    logging.warning("⚠️ No sheets detected!")
